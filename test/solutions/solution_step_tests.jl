...
@testset "$(rpad("ODE Solution Step",80))" begin
    ...
    cut_periodic_solution!(solstep, (q = [2π, 0.0],))
    @test solstep.q == [0.0, 2π]
    ...
    @test solstep.q̄ ≈
          [A * sin(- ω * timestep(ode) + ϕ), ω * A * cos(- ω * timestep(ode) + ϕ)]
end
...
@testset "$(rpad("DAE Solution Step",80))" begin
    solstep = SolutionStepDAE(t0, StateVariable(x0), AlgebraicVariable(λ0),
        AlgebraicVariable(μ0), parameters(dae))
    ...
    @test solstep.q̄[1:2] ≈
          [A * sin(- ω * timestep(dae) + ϕ), ω * A * cos(- ω * timestep(dae) + ϕ)]
end

@testset "$(rpad("PDAE Solution Step",80))" begin
    solstep = SolutionStepPDAE(t0, StateVariable(q0), StateVariable(p0),
        AlgebraicVariable(λ0), AlgebraicVariable(μ0), parameters(pdae))
    ...
    @test previous(solstep) ==
          (t = zero(t0), q = zero(q0), p = zero(p0), λ = zero(λ0), μ = zero(μ0))
