...
function energy_change(sol, params)
    abs(hamiltonian(sol[end].t, sol[end].q, params) -
...
@testset "Projected integrator convergence and energy conservation" begin
    for (P, name, etol) in ((PostProjection, "PostProjection", 1),
        ...
        @testset "$name" begin
            test_convergence_order(build, P(Gauss(1)), steps(10, 4); reference = ref,
                errormetric = emq, expected = 2, label = "$name(Gauss(1))")
            test_convergence_order(build, P(Gauss(2)), steps(5, 4); reference = ref,
                errormetric = emq, expected = 4, label = "$name(Gauss(2))")
            test_convergence_order(build, P(Gauss(3)), steps(3, 3); reference = ref,
                errormetric = emq, expected = 6, label = "$name(Gauss(3))")
