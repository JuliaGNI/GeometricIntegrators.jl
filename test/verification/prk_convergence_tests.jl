...
@testset "Partitioned Runge-Kutta convergence" begin
    test_convergence_order(build, SymplecticEulerARK(), steps(20, 4); reference = href,
        errormetric = emq, expected = 1, label = "SymplecticEulerARK")
    test_convergence_order(build, SymplecticEulerBRK(), steps(20, 4); reference = href,
        errormetric = emq, expected = 1, label = "SymplecticEulerBRK")
    test_convergence_order(build, PartitionedGauss(2), steps(5, 4); reference = href,
        errormetric = emq, expected = 4, label = "PartitionedGauss(2)")
    test_convergence_order(build, LobattoIIIAIIIB(2), steps(5, 4); reference = href,
        errormetric = emq, expected = 2, label = "LobattoIIIAIIIB(2)")
    test_convergence_order(build, LobattoIIIBIIIA(2), steps(5, 4); reference = href,
        errormetric = emq, expected = 2, label = "LobattoIIIBIIIA(2)")
    test_convergence_order(build, LobattoIIIAIIIB(3), steps(5, 3); reference = href,
        errormetric = emq, expected = 4, label = "LobattoIIIAIIIB(3)")
