...
function RungeKutta.check_symplecticity(tab::TableauSPARK{T}; atol = 16 * eps(T), rtol = 16 *
                                                                                         eps(T)) where {T}
    ...
    s_ω = [isapprox(tab.ω[i, j], (i == j ? 1 : 0); atol = atol, rtol = rtol)
           for i in 1:tab.s, j in 1:(tab.s + 1)]
...
function solversize(method::SPARKMethod, problem::AbstractProblemIDAE)
...
```
"""
const IntegratorSPARK{DT, TT} = GeometricIntegrator{
    <:SPARKMethod, <:Union{IDAEProblem{DT, TT}, LDAEProblem{DT, TT}}}
...
function initial_guess!(sol, history, params,
        int::GeometricIntegrator{<:SPARKMethod, <:Union{IDAEProblem, LDAEProblem}})
    ...
    for i in 1:pstages(method(int))
        ...
        for k in 1:D
            ...
            C.x[2 * D * nstages(int) + 3 * (D * (i - 1) + k - 1) + 2] = (C.Pp[i][k] -
                                                                         sol.p[k]) /
                                                                        timestep(int)
            C.x[2 * D * nstages(int) + 3 * (D * (i - 1) + k - 1) + 3] = C.Vp[i][k]
        end
    end
    ...
    #     for k in 1:D
    #         C.x[2*D*nstages(int)+3*(k-1)+1] = C.λ[k]
    #     end
    # end

    if hasnullvector(method(int))
        for k in 1:D
            C.x[2 * D * nstages(int) + 3 * D * pstages(method(int)) + k] = 0
        end
    end
end
...
function components!(x::AbstractVector{ST},
        sol,
        params,
        int::GeometricIntegrator{<:SPARKMethod, <:Union{IDAEProblem, LDAEProblem}}) where {ST}
...
function residual!(b::AbstractVector{ST},
    ...
    for i in 1:R
        for k in 1:D
            ...
            for j in 1:S
                b[2 * D * S + 3 * (D * (i - 1) + k - 1) + 2] += tableau(int).p̃.a[i, j] *
                                                                C.Fi[j][k]
            end
            for j in 1:R
                b[2 * D * S + 3 * (D * (i - 1) + k - 1) + 1] += tableau(int).q̃.α[i, j] *
                                                                C.Up[j][k]
                b[2 * D * S + 3 * (D * (i - 1) + k - 1) + 2] += tableau(int).p̃.α[i, j] *
                                                                C.Gp[j][k]
            end
        end
    end
    ...
    for i in 1:(R - P)
        for k in 1:D
            for j in 1:R
                b[2 * D * S + 3 * (D * (i - 1) + k - 1) + 3] -= tableau(int).ω[i, j] *
                                                                C.Φp[j][k]
            end
            b[2 * D * S + 3 * (D * (i - 1) + k - 1) + 3] -= tableau(int).ω[i, R + 1] *
                                                            C.ϕ̃[k]
        end
    end
    ...
    for i in (R - P + 1):R
        for k in 1:D
            for j in 1:R
                b[2 * D * S + 3 * (D * (i - 1) + k - 1) + 3] -= tableau(int).δ[j] *
                                                                C.Vp[j][k]
            end
        end
    end
    ...
    if hasnullvector(method(int))
        for i in 1:R
            for k in 1:D
                b[2 * (D * (i - 1) + k - 1) + 3] -= C.μ[k] * tableau(int).d[i] /
                                                    tableau(int).p.b[i]
            end
        end
