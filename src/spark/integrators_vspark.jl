...
function initial_guess!(int::GeometricIntegrator{<:VSPARK,<:Union{IDAEProblem,LDAEProblem}})
    ...
    for i in 1:nstages(int)
        ...
        for k in 1:ndims(int)
            C.x[3*(ndims(int)*(i-1)+k-1)+1] = (C.Qi[i][k] - solstep(int).q̄[k]) / timestep(int)
            C.x[3*(ndims(int)*(i-1)+k-1)+2] = (C.Pi[i][k] - solstep(int).p̄[k]) / timestep(int)
