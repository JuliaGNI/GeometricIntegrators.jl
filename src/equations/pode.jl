...
immutable PODE{T,FQ,FP} <: Equation{T}
    d::Int
    n::Int
    f::FQ
    g::FP
...
function PODE{T,FQ,FP}(f::FQ, g::FP, t₀::Real, q₀::DenseArray{T}, p₀::DenseArray{T})
    @assert size(q₀) == size(p₀)
    PODE{T,FQ,FP}(size(q₀, 1), size(q₀, 2), f, g, t₀, q₀, p₀)
end
...
Base.:(==){T1, T2, FQ1, FQ2, FP1, FP2}(ode1::PODE{T1,FQ1,FP1}, ode2::PODE{T2,FQ2,FP2}) = (
                                ode1.d == ode2.d
                             && ode1.n == ode2.n
                             && ode1.f == ode2.f
                             && ode1.g == ode2.g
