...
# function get_initial_conditions!(sol::SolutionPDAE{AT,TT}, asol::AtomicSolutionPDAE{DT,TT,AT}, k, n = 1) where {DT,TT,AT} # TODO
# function get_initial_conditions!(sol::SolutionPDAE, asol::AtomicSolutionPDAE, k, n = 1)
...
# function get_solution!(sol::SolutionPDAE{AT}, q::AT, p::AT, λ::AT, n, k = 1) where {AT} # TODO
function get_solution!(sol::SolutionPDAE, q, p, λ, n)
    q .= sol.q[n]
    p .= sol.p[n]
    λ .= sol.λ[n]
end

# function get_solution!(sol::SolutionPDAE{AT}, q::AT, p::AT, n, k = 1) where {AT} # TODO
function get_solution!(sol::SolutionPDAE, q, p, n)
    q .= sol.q[n]
    p .= sol.p[n]
end

# function get_solution(sol::SolutionPDAE{AT,TT}, n, k = 1) where {AT,TT}
