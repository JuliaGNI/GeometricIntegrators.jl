...
"""
function create_hdf5(solution::SolutionPSDE{DT,TT,NQ,NW}, file::AbstractString; save_W=true) where {DT,TT,NQ,NW}
    # create HDF5 file
    ...
    elseif NQ==3
        ...
        p = d_create(solution.h5, "p", datatype(DT), dataspace(solution.nd, solution.nt+1, solution.ns), "chunk", (solution.nd,1,1))
        # copy initial conditions
        q[:, 1, :] = solution.q[:, 0, :]
        p[:, 1, :] = solution.p[:, 0, :]
    end
...
"""
function CommonFunctions.write_to_hdf5(solution::SolutionPSDE, h5::HDF5File=hdf5(solution), offset=0, offset2=offset)
...
function copy_solution_to_hdf5(solution::SolutionPSDE{DT,TT,2,NW}, h5::HDF5File, j1, j2, n1, n2) where {DT,TT,NW}
    h5["q"][:, j1:j2] = solution.q[:, n1:n2]
    h5["p"][:, j1:j2] = solution.p[:, n1:n2]
end

function copy_solution_to_hdf5(solution::SolutionPSDE{DT,TT,3,NW}, h5::HDF5File, j1, j2, n1, n2) where {DT,TT,NW}
    h5["q"][:, j1:j2, :] = solution.q[:, n1:n2, :]
    h5["p"][:, j1:j2, :] = solution.p[:, n1:n2, :]
end

function copy_increments_to_hdf5(solution::SolutionPSDE{DT,TT,NQ,2}, h5::HDF5File, j1, j2, n1, n2) where {DT,TT,NQ}
    h5["ΔW"][:,j1:j2] = solution.W.ΔW[:,n1:n2]
    h5["ΔZ"][:,j1:j2] = solution.W.ΔZ[:,n1:n2]
end

function copy_increments_to_hdf5(solution::SolutionPSDE{DT,TT,NQ,3}, h5::HDF5File, j1, j2, n1, n2) where {DT,TT,NQ}
    h5["ΔW"][:,j1:j2,:] = solution.W.ΔW[:,n1:n2,:]
    h5["ΔZ"][:,j1:j2,:] = solution.W.ΔZ[:,n1:n2,:]
end
