...
struct IntegratorVSPARK{DT, TT, D, S, R, PT <: ParametersVSPARK{DT,TT,D,S,R},
    ...
    function IntegratorVSPARK(equation::Union{IDAEProblem{DT}, LDAEProblem{DT}}, tableau::Union{TableauSPARK,TableauVSPARK}, Δt=tstep(equation); kwargs...) where {DT}
        IntegratorVSPARK{DT, ndims(equation)}(functions(equation), tableau, Δt; kwargs...)
    end
end


GeometricBase.nconstraints(::IntegratorVSPARK{DT,TT,D}) where {DT,TT,D} = D
...
function compute_stages!(x::Vector{ST}, cache::IntegratorCacheSPARK{ST,D,S,R},
    ...
    for i in 1:R
        ...
        params.equs[:u](cache.Up[i], tλᵢ, cache.Qp[i], cache.Vp[i], cache.Pp[i], cache.Λp[i])
