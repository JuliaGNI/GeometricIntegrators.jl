...
function IntegratorHSPARKprimary(equation::PDAE{DT,TT,VT,FT,UT,GT,ϕT},
    ...
    # create params
    params = ParametersHSPARKprimary{DT,TT,D,S,R,P,VT,FT,UT,GT,ϕT}(
    ...
    quote
        ...
        for i in 1:R-P
            for k in 1:D
                ...
                b[2*D*S+3*(D*(i-1)+k-1)+3] -= params.t_ω[i,R+1] * $cache.ϕ̃[k]
            end
        end
        ...
        for i in R-P+1:R
            for k in 1:D
                ...
                for j in 1:R
                    b[2*D*S+3*(D*(i-1)+k-1)+3] -= params.t_δ[j] * $cache.Λp[j][k]
                end
            end
        end
    end
