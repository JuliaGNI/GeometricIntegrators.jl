...
function initialize!(ig::InitialGuessIODE{TT},
    ...
    extrapolate_iode!(t₀, q₀, p₀, t₁, q₁, p₁, ig.v, ig.f, MidpointExtrapolation(ig.s))
...
function Solutions.update_vector_fields!(ig::InitialGuessIODE{TT}, t₁::TT,
                               q₁::SolutionVector{DT},
                               p₁::SolutionVector{DT},
                               v₁::Vector{DT},
...
function GeometricBase.evaluate!(ig::InitialGuessIODE{TT},
    ...
    if q₀ == q₁
        ...
        extrapolate!(zero(TT), q₀, v₀, ig.Δt, q₁, v₁, (one(TT)+c_q)*ig.Δt, guess_q, ig.int)
    end
    ...
    if p₀ == p₁
        ...
        extrapolate!(zero(TT), p₀, f₀, ig.Δt, p₁, f₁, (one(TT)+c_p)*ig.Δt, guess_p, ig.int)
    end
end
...
function GeometricBase.evaluate!(ig::InitialGuessIODE{TT},
    ...
    if q₀ == q₁
        ...
        extrapolate!(zero(TT), q₀, v₀, ig.Δt, q₁, v₁, (one(TT)+c_q)*ig.Δt, guess_q, guess_v, ig.int)
    end
    ...
    if p₀ == p₁
        ...
        extrapolate!(zero(TT), p₀, f₀, ig.Δt, p₁, f₁, (one(TT)+c_p)*ig.Δt, guess_p, guess_f, ig.int)
    end
end