...
"""
struct IntegratorDGVIEXP{DT,TT,D,S,R,ΘT,FT,GT,HT,VT,FPT,ST,IT,BT<:Basis} <: DeterministicIntegrator{DT,TT}
...
function IntegratorDGVIEXP(equation::IODE{DT,TT,ΘT,FT,GT,HT,VT}, basis::Basis{TT,P},
    ...
    # create integrator
    IntegratorDGVIEXP{DT, TT, D, S, R, ΘT, FT, GT, HT, VT, typeof(params), typeof(solver),
                typeof(iguess.int), typeof(basis)}(
                equation, basis, quadrature, params, solver, iguess,
                q, q⁻, q⁺, cache)
end

@inline equation(integrator::IntegratorDGVIEXP) = integrator.equation
...
function initial_guess!(int::IntegratorDGVIEXP{DT,TT, D, S, R}, sol::AtomicSolutionPODE{DT,TT}) where {DT,TT,D,S,R}
...
function integrate_step!(int::IntegratorDGVIEXP{DT,TT}, sol::AtomicSolutionPODE{DT,TT}) where {DT,TT}
