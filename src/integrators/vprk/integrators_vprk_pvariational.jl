...
function NonlinearFunctionParametersVPRKpVariational{DT,TT,AT,GT}(α::AT, g::GT, Δt::TT, o::Int, R::Vector, q::Vector{DT}, p::Vector{DT})
    @assert length(q)==length(p)
    R  = convert(Vector{TT}, R)
    R1 = [one(TT), zero(TT)]
    R2 = [zero(TT), one(TT)]
    NonlinearFunctionParametersVPRKpVariational{DT,TT,AT,GT,length(q)}(α, g, Δt, o, R, R1, R2, 0, q, p)
end


@generated function compute_projection_vprk!{ST,DT,TT,ΑT,GT,D}(x::Vector{ST}, q̅::Vector{ST}, p̅::Vector{ST}, λ::Vector{ST}, U::Matrix{ST}, G::Matrix{ST}, params::NonlinearFunctionParametersVPRKpVariational{DT,TT,ΑT,GT,D})
...
@generated function function_stages!{ST,DT,TT,ΑT,GT,D}(x::Vector{ST}, b::Vector{ST}, params::NonlinearFunctionParametersVPRKpVariational{DT,TT,ΑT,GT,D})
...
immutable IntegratorVPRKpVariational{DT, TT, ΑT, FT, GT, VT, SPT, PPT, SST, STP, IT} <: AbstractIntegratorVPRK{DT, TT}
...
function IntegratorVPRKpVariational{DT,TT,ΑT,FT,GT,VT}(equation::IODE{DT,TT,ΑT,FT,GT,VT}, tableau::TableauVPRK{TT}, Δt::TT;
    ...
    # R[2] *= tableau.R∞
    pparams = NonlinearFunctionParametersVPRKpVariational(equation.α, equation.g, Δt, tableau.o, R, q, p)
...
function integrate!{DT,TT,ΑT,FT,GT,VT,N}(int::IntegratorVPRKpVariational{DT,TT,ΑT,FT,GT,VT}, sol::SolutionPDAE{DT,TT,N}, m1::Int, m2::Int)
    @assert m1 ≥ 1
    @assert m2 ≤ sol.ni
    ...
    for m in m1:m2
        ...
        for n in 1:sol.ntime
            ...
            if !check_solver_status(int.projector.status, int.projector.params)
