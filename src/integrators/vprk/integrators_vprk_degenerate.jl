...
function Integrators.IntegratorCache(
        params::ParametersVPRKdegenerate{DT, TT, D, S}; kwargs...) where {DT, TT, D, S}
    IntegratorCacheVPRK{DT, D, S}(D * S, true, D; kwargs...)
end

function Integrators.IntegratorCache{ST}(
        params::ParametersVPRKdegenerate{DT, TT, D, S}; kwargs...) where {ST, DT, TT, D, S}
...
mutable struct IntegratorVPRKdegenerate{DT, TT, D, S,
    ...
    function IntegratorVPRKdegenerate(params::ParametersVPRK{DT, TT, D, S},
            ...
            solver::ST, projector::PST, iguess::IT, caches) where {
            DT, TT, D, S, ST, PST, IT}
        new{DT, TT, D, S, typeof(params), typeof(pparams), ST, PST, IT}(
            params, pparams, solver, projector, iguess, caches)
    end

    function IntegratorVPRKdegenerate{DT, D}(
            equations::NamedTuple, tableau::PartitionedTableau{TT},
            nullvec, Δt::TT; R = [1, 1]) where {DT, TT, D}
        ...
        pparams = ParametersVPRKdegenerate{DT, D}(equations,
            tableau,
            nullvec,
            Δt,
            NamedTuple{(:V, :F)}((create_internal_stage_vector(DT, D, S),
    ...
    function IntegratorVPRKdegenerate(problem::Union{IODEProblem{DT}, LODEProblem{DT}},
            tableau, nullvec; kwargs...) where {DT}
        IntegratorVPRKdegenerate{DT, ndims(problem)}(
...
function initial_guess_projection!(
        int::IntegratorVPRKdegenerate{DT}, sol::SolutionStepPODE{DT},
        cache::IntegratorCacheVPRK{DT} = int.caches[DT]) where {DT}
    for k in eachdim(int)
        cache.x̄[k] = sol.q[k]
    end
...
function Integrators.residual!(x::Vector{ST}, b::Vector{ST},
    ...
    compute_solution!(x, cache.q̃, cache.ṽ, cache.p̃, params)
    ...
    for k in 1:div(D, 2)
        ...
        for i in 1:S
            b[div(D, 2) + k] += timestep(problem) * tableau(method).p.b[i] *
                                params.pparams[:F][i][k]
        end
    end
end

function integrate_step!(
        int::IntegratorVPRKdegenerate{DT, TT}, sol::SolutionStepPODE{DT, TT},
    ...
    compute_solution!(cache.x̄, sol.q, cache.ṽ, sol.p, int.pparams)
end
