...
function components!(x::AbstractVector{ST}, sol, params,
        int::GeometricIntegrator{<:HPImidpoint, <:AbstractProblemIODE}) where {ST}
    ...
    method(int).D₁ϕ(
        cache(int, ST).D₁ϕ, sol.q, cache(int, ST).q, cache(int, ST).a, timestep(int))
    method(int).D₂ϕ(
        cache(int, ST).D₂ϕ, sol.q, cache(int, ST).q, cache(int, ST).a, timestep(int))
    method(int).Dₐϕ(
        cache(int, ST).Dₐϕ, sol.q, cache(int, ST).q, cache(int, ST).a, timestep(int))
