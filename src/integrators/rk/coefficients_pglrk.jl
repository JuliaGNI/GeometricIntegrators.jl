...
function CoefficientsPGLRK(name::Symbol, order::Int, a::Matrix{T}, b::Vector{T},
        c::Vector{T}, P::Matrix{T}, X::Matrix{T}, W::Matrix{T}) where {T}
...
function GeometricBase.description(::CoefficientsPGLRK)
    "Projected Gauss-Legendre Runge-Kutta coefficients"
end

RungeKutta.nstages(coeff::CoefficientsPGLRK) = coeff.s
RungeKutta.eachstage(coeff::CoefficientsPGLRK) = 1:coeff.s

function Base.hash(tab::CoefficientsPGLRK, h::UInt)
    hash(tab.o,
        hash(tab.s,
            hash(tab.a,
                hash(tab.b, hash(tab.c, hash(tab.P, hash(tab.Q, hash(tab.X, hash(tab.W, hash(tab.A, h))))))))))
end

function Base.:(==)(tab1::CoefficientsPGLRK, tab2::CoefficientsPGLRK)
    (tab1.o == tab2.o
     && tab1.s == tab2.s
     && tab1.a == tab2.a
     && tab1.b == tab2.b
...
function Base.isequal(tab1::CoefficientsPGLRK{T1}, tab2::CoefficientsPGLRK{T2}) where {
        T1, T2}
    (tab1 == tab2 && T1 == T2 && typeof(tab1) == typeof(tab2))
end

function Base.isapprox(tab1::CoefficientsPGLRK, tab2::CoefficientsPGLRK; kwargs...)
    (tab1.o == tab2.o
     && tab1.s == tab2.s
...
function Base.show(io::IO, tab::CoefficientsPGLRK)
    print(io, "Projected Gauss-Legendre Runge-Kutta Coefficients ",
        tab.name, " with ", tab.s, " stages and order ", tab.o, "\n")
    print(io, "  a = ", tab.a, "\n")
    print(io, "  b = ", tab.b, "\n")
    print(io, "  c = ", tab.c, "\n")
    print(io, "  P = ", tab.P, "\n")
