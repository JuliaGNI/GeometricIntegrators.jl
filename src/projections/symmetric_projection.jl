...
struct SymmetricProjectionCache{DT,D,M,N} <: IODEIntegratorCache{DT,D}
    x::Vector{DT}
    ...
    function SymmetricProjectionCache{DT,D,M,N}() where {DT,D,M,N}
        x = zeros(DT, N+D+M)
        x̄ = @view x[1:N]
        x̃ = @view x[N+1:N+D+M]
...
nlsolution(cache::SymmetricProjectionCache) = cache.x

function current(cache::SymmetricProjectionCache, solstep::Union{SolutionStepODE,SolutionStepDAE})
    (t = solstep.t, q = cache.q)
end

function current(cache::SymmetricProjectionCache, solstep::Union{SolutionStepPODE,SolutionStepPDAE})
    (t = solstep.t, q = cache.q, p = cache.p)
end

function reset!(cache::SymmetricProjectionCache, t, q)
    copyto!(cache.q, q)
end

function reset!(cache::SymmetricProjectionCache, t, q, p, λ = missing)
    copyto!(cache.q, q)
    copyto!(cache.p, p)
end

function update!(cache::SymmetricProjectionCache, V::AbstractVector, tableau::Tableau, Δt::Number)
    update_vector!(cache.Δq, V, tableau, Δt)
    cache.q .+= cache.Δq
end

function update!(cache::SymmetricProjectionCache, V::AbstractVector, F::AbstractVector, tableau::Tableau, Δt::Number)
    update_vector!(cache.Δq, V, tableau, Δt)
...
function update!(cache::SymmetricProjectionCache, V::AbstractVector, F::AbstractVector, tableau::PartitionedTableau, Δt::Number)
...
function update!(cache::SymmetricProjectionCache, subcache::Union{ODEIntegratorCache,DAEIntegratorCache}, tableau::Union{Tableau,PartitionedTableau}, Δt::Number)
    update!(cache, subcache.V, tableau, Δt)
end

function update!(cache::SymmetricProjectionCache, subcache::Union{IODEIntegratorCache,PODEIntegratorCache}, tableau::Union{Tableau,PartitionedTableau}, Δt::Number)
    update!(cache, subcache.V, subcache.F, tableau, Δt)
end

function project!(cache::SymmetricProjectionCache, U::AbstractVector, G::AbstractVector, Δt::Number)
    cache.q .+= Δt .* U
    cache.p .+= Δt .* G
end


function Cache{ST}(problem::GeometricProblem, method::ProjectedMethod{<:SymmetricProjection}; kwargs...) where {ST}
    SymmetricProjectionCache{ST, ndims(problem), nconstraints(problem), solversize(problem, parent(method))}(; kwargs...)
end

@inline CacheType(ST, problem::GeometricProblem, method::ProjectedMethod{<:SymmetricProjection}) = 
    SymmetricProjectionCache{ST, ndims(problem), nconstraints(problem), solversize(problem, parent(method))}
...
const IntegratorSymmetricProjection{DT,TT} = Integrator{<:GeometricProblem{DT,TT}, <:ProjectedMethod{<:SymmetricProjection}}
...
function components!(
    ...
    functions(problem).u(U[1], solstep.t̄[1], solstep.q̄[1], λ)
...
function components!(
    ...
    # compute g = ∇ϑ(q)⋅λ
    functions(problem).g(G[1], solstep.t̄[1], solstep.q̄[1], solstep.v̄[1], λ)
...
function constraint!(solstep::SolutionStep, problem::DAEProblem, cache::SymmetricProjectionCache)
    # compute ϕ = ϕ(q)
    functions(problem).ϕ(cache.ϕ, solstep.t, cache.q)
end


function constraint!(solstep::SolutionStep, problem::Union{IODEProblem,LODEProblem}, cache::SymmetricProjectionCache)
    # compute ϕ = ϑ(q) - p
...
function integrate_step!(int::Integrator{<:GeometricProblem, <:ProjectedMethod{<:SymmetricProjection}})
