...
function initmethod(projection::ProjectionMethod, method::GeometricMethod)
    ProjectedMethod(projection, method)
end
...
function ProjectionIntegrator(
    ...
    solver = initsolver(solvermethod, method, caches;
        (isempty(options) ? default_options(projectionmethod, problem) : options)...)
...
function ProjectionIntegrator(
    ...
    subint = GeometricIntegrator(problem, parent(projectionmethod), parent_solvermethod,
        parent_iguess; default_options(parent(projectionmethod), problem)...)
...
function GeometricIntegrator(
    ...
    ProjectionIntegrator(problem, method, solver, initialguess,
        parent_solver, parent_initialguess; kwargs...)
end
...
function project!(sol, U::AbstractVector, G::AbstractVector,
        int::ProjectionIntegrator{<:ProjectionMethod, <:DAEProblem})
    sol.q .+= timestep(int) .* U
end

function project!(sol,
        U::AbstractVector,
        G::AbstractVector,
        int::ProjectionIntegrator{<:ProjectionMethod,
            <:Union{IODEProblem, LODEProblem, PDAEProblem, HDAEProblem}})
    sol.q .+= timestep(int) .* U
